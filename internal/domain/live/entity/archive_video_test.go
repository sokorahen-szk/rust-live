...
func Test_NewArchiveVideo(t *testing.T) {
	...
	status := NewVideoStatus(VideoStatusEnded)
	...
	archiveVideo := NewArchiveVideo(id, broadcastId, title, url, stremer, platform, status, thumbnailImage, startedDatetime, endedDatetime)
	...
	a.Equal(status, archiveVideo.GetStatus())
